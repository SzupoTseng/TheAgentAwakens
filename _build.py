...
# -*- coding: utf-8 -*-
"""組裝《代理覺醒》成品:
  1) 合併單一 Markdown 檔  代理覺醒_全書.md
  2) 圖文 HTML(側邊子目錄)  代理覺醒.html
版面沿用 D:/GameDevZ/gs2/docs/book 的 _build.py(同一套 CSS / 解析器),
內容為技術書(龍蝦 OpenClaw vs Hermes Agent + SRE 實戰百景)。
...
BOOK_TITLE = "代理覺醒"
BOOK_SUB   = "龍蝦 OpenClaw 與 Hermes Agent，以及 SRE／Fab／Lab 實戰百景"
...
def build_merged():
    ...
    if os.path.exists(os.path.join(BOOK, "TheAgentAwakens.png")):
        parts.append("![代理覺醒 · The Agent Awakens — 封面海報](TheAgentAwakens.png)\n")
    parts.append("> 全書合併版。內容彙整自一份公開技術搜尋資料(OpenClaw / Hermes Agent 比較 + SRE/Fab 實戰情境)。\n")
    parts.append("\n---\n\n## 目錄\n")
    for base in FILES:
...
def cover_section():
    p = os.path.join(BOOK, "TheAgentAwakens.png")
